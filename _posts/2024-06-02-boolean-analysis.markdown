...
Define an inner product on pairs of function $f,g:\\{-1,1\\}^n:=G_n\to\mathbb R$ by 
...
If $\sum_{x\in G_n} f(x)=0$, then we have the following Boolean Poincare Inequality

$$
\sum_{i=1}^n \frac{1}{|G_n|}\sum_{x\in G_n} |D_i f(x)|^2 \geq \frac{1}{|G_n|}\sum_{x\in G_n}|f(x)|^2 .
$$

The proof is very straightforward using Fourier analysis.
